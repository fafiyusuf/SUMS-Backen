...
async function startServer() {
  ...
  try {
    ...
    if (config.env === 'production') {
      console.log('3. Production environment detected. Skipping alter sync...');
      await sequelize.sync();
      logger.info('Database synchronized (production schema check)');
    } else {
      console.log('3. Syncing database (alter enabled for dev)...');
